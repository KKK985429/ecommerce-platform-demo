...
def test_bug_payment_gateway_contract_key_error_fixed(monkeypatch) -> None:
    ...
    assert result["discount_amount"] == Decimal("29.00")
    assert result["tax_amount"] == Decimal("20.88")
    assert result["final_amount"] == Decimal("281.87")
